        ...
        loop {
            tokio::select! {
                ...
                Some((task, key, sent_time, client_ch, action)) = lru_rx.recv() => {
                        ...
                        match action {
                            LruAction::Attach => {
                                println!("{} delay {:?} LRU: action Attach {:?}",task, Instant::now().duration_since(sent_time).as_nanos(), key);
                                lru_evict.attach(task, key, cache.clone()).await;
                            }
                            LruAction::Move_to_head => {
                                println!("{} delay {:?} LRU: action move_to_head {:?}",task, Instant::now().duration_since(sent_time).as_nanos(), key);
                ...
                Some(client_ch) = lru_flush_rx.recv() => {
                                ...
                                println!("LRU service - flush lru ");
                                    ...
                                    while let Some(entry_) = entry {
                                            lc += 1;     
                                            let key = entry_.lock().await.key.clone();
                                            println!("LRU: flush-persist  {}  {:?}",lc, key);
